...
- first-function - Code for the application's Lambda function and Project Dockerfile.
- events - Invocation events that you can use to invoke the function.
- first-function/tests - Unit tests for the application code.
...
```bash
lambda-docker-example$ sam build
```
...
```bash
lambda-docker-example$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
lambda-docker-example$ sam local start-api
lambda-docker-example$ curl http://localhost:3000/
```
...
```bash
lambda-docker-example$ sam logs -n HelloWorldFunction --stack-name lambda-docker-example --tail
```
...
```bash
lambda-docker-example$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name lambda-docker-example
```
